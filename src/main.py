...
if __name__ == '__main__':
    ...
    model = CM_HGNN(args, num_items, num_cat, cat4item)
    ...
    for e in range(args.epoch):
        ...
        for batch_data in bar:
            ...
            scores = model(dict((k,v.to(args.device)) if isinstance(v,torch.LongTensor) else (k,v)  for k,v in batch_data.items()))

            optimizer.zero_grad() 
            loss = model.loss_function(scores, batch_data['target'].to(args.device)) 
            loss.backward()
            optimizer.step()
            all_loss += loss.item()
            mini_batch += 1
            bar.set_postfix(Epoch=e, Train_Loss=loss.item()/batch_data['target'].size(0))
        scheduler.step()
        ...
        test_num = 0
        for batch_data in tqdm(test_loader,ncols=80,desc='test'):
            scores = model(dict((k,v.to(args.device)) if isinstance(v,torch.LongTensor) else (k,v)  for k,v in batch_data.items()))
            loss = model.loss_function(scores, batch_data['target'].to(args.device)) 
            all_loss += loss.item()
            for k in hr.keys():
                this_hr, this_mrr = topk_mrr_hr(scores.detach().cpu(),batch_data['target'].numpy(),k)
                hr[k] += this_hr
                mrr[k] += this_mrr
            test_num += batch_data['target'].shape[0]
